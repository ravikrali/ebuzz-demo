...
(function () {
  ...
  const base = { rating: null, product: '', sku: '', vendor: '', emoji: '', promo_price: null, list_price: null, cta: '', sponsored: 0, status: 'published', likes: 0, shares: 0, comments_json: '[]', mod_score: 0.02, mod_flags: '', impressions: 0, clicks: 0, targets: '', incentivized: 0, verified: 0, resolution: '' };
  const SEED = [
    ...
    { id: 'fp-12', author_id: 'u_61aa7', author: 'Dev P.', avatar: '👨🏾', kind: 'review', text: 'The Sitwell memory-foam cushion is comfy for the first hour, but it flattened a bit after two weeks. Is that normal?', rating: 3, product: 'Sitwell Memory Foam Seat Cushion', sku: 'cush', vendor: 'Sitwell Home', emoji: '🟫', likes: 12, verified: 1, created_at: ago(7) },
    { id: 'fp-13', author_id: 'u_2e7b0', author: 'Nina W.', avatar: '👩🏻', kind: 'feedback', text: 'Love the Deal Room! Could the offer timer also show on my phone lock screen? I missed a great offer while cooking 😅', likes: 41, created_at: ago(12) },
    { id: 'fp-14', author_id: 'u_90d4c', author: 'Leo F.', avatar: '🧑🏼', kind: 'feedback', text: 'Please let me see products as a simple list, not just cards. Easier to compare prices and ratings.', likes: 88, resolution: 'Shipped', created_at: ago(70), comments_json: JSON.stringify([{ by: 'eBuzz Team', brand: 1, team: 1, text: 'Great idea, Leo. List view is live today: use the Cards / List switch above any product results 🙌' }]) },
    { id: 'fp-15', author_id: 'brand_ebuzz', author: 'eBuzz Team', avatar: '🍯', kind: 'update', text: 'New this week: ☰ List view for product results (with a Match score), brand storefront pages you can open from any vendor name, and faster replies from sellers on your reviews.', likes: 156, shares: 22, created_at: ago(4) },
    { id: 'fp-16', author_id: 'brand_sitwell', author: 'Sitwell Home', avatar: '🪑', kind: 'promo', sponsored: 1, text: 'Firm support without the premium price. Sitwell Aria, now with a free memory-foam cushion for Buzz Feed readers.', product: 'Sitwell Aria', sku: 'sitwell', vendor: 'Sitwell Home', emoji: '🪑', promo_price: 239, list_price: 259, cta: 'Get offer', likes: 27, impressions: 11240, clicks: 402, targets: 'Back pain,Tailbone pain', created_at: ago(30) },
  ].map((p) => ({ ...base, ...p }));
  /* idempotent: adds any seed post this device doesn't have yet (safe to call on every load) */
  F.seed = () => { const miss = SEED.filter((p) => !D().get('feed_posts', p.id)); return miss.length ? D().put('feed_posts', miss, { silent: true }) : Promise.resolve(); };
  F.isTeam = (p) => p.author_id === 'brand_ebuzz';
  F.brandReplied = (p, name) => { try { return JSON.parse(p.comments_json || '[]').some((c) => c.brand && c.by === name); } catch { return false; } };
  ...
  F.card = (p, o = {}) => {
    ...
    const kindTag = { review: 'Review', story: 'Story', tip: 'Tip', question: 'Question', promo: 'Promotion', feedback: 'Platform feedback', update: 'Platform update' }[p.kind] || p.kind;
    const RES = { Acknowledged: '', Planned: 'blue', 'In progress': 'honey', Shipped: 'green', "Won't do": 'red' };
    const el = h(`<article class="post ${isPromo ? 'promo' : ''} ${p.kind === 'update' ? 'update' : ''} ${p.status !== 'published' ? 'dim' : ''}" data-post="${p.id}">
      <header class="row" style="gap:10px"><div class="post-av">${p.avatar || '🙂'}</div>
        <div style="flex:1;min-width:0"><div class="row wrap" style="gap:6px"><b>${esc(p.author)}</b>${F.isTeam(p) ? '<span class="tag violet">✓ Official</span>' : p.author_id.startsWith('brand_') ? '<span class="tag blue">Brand</span>' : ''}${p.kind === 'update' ? '<span class="tag honey">📣 Platform update</span>' : ''}${p.kind === 'feedback' && p.resolution ? `<span class="tag ${RES[p.resolution] || ''}">${esc(p.resolution)}</span>` : ''}${+p.verified ? '<span class="tag green">✓ Verified purchase</span>' : ''}${+p.incentivized ? '<span class="tag">Received product free</span>' : ''}${isPromo ? '<span class="tag honey">Sponsored</span>' : ''}</div>
        ...
        ${o.moderation ? `<span class="tag ${p.mod_score > 0.5 ? 'red' : 'green'}" title="AI moderation risk score">risk ${Math.round(p.mod_score * 100)}</span>` : o.noReport ? '' : `<button class="icon-btn" data-report title="Report" aria-label="Report post" style="width:30px;height:30px">⚑</button>`}</header>
      ...
      ${p.product ? `<div class="post-prod"><div class="post-art">${p.emoji || '📦'}</div><div style="flex:1;min-width:0"><b style="font-size:13.5px">${esc(p.product)}</b><div class="muted" style="font-size:12px">${o.onVendor && p.vendor ? `<a href="#" data-vend>${esc(p.vendor)}</a>` : esc(p.vendor)}${isPromo && p.promo_price ? ` · <b style="color:var(--green)">${money(p.promo_price, 0)}</b> <s>${money(p.list_price, 0)}</s>` : ''}</div></div>${o.onShop && (p.sku || p.cta === 'Visit store') ? `<button class="btn sm ${isPromo ? 'primary' : ''}" data-shop>${isPromo ? esc(p.cta || 'Get offer') : 'View'}</button>` : ''}</div>` : ''}
      ...
      <div class="post-comments" hidden>${cm.map((c) => `<div class="cm"><b>${esc(c.by)}</b>${c.team ? ' <span class="tag violet">eBuzz Team</span>' : c.brand ? ' <span class="tag blue">Brand</span>' : ''} ${esc(c.text)}</div>`).join('')}
        ${o.commentAs ? `<div class="row" style="margin-top:6px"><input class="btn sm" style="flex:1;text-align:left;border-radius:10px" placeholder="${o.commentAs.brand ? 'Reply publicly as ' + esc(o.commentAs.name) + '…' : 'Add a comment…'}"><button class="btn sm primary" data-send>Post</button></div>` : ''}</div>
    </article>`);
    ...
    const send = $('[data-send]', el); if (send) send.onclick = async () => { const i = $('input', el); if (!i.value.trim()) return; cm.push({ by: o.commentAs.name, brand: o.commentAs.brand ? 1 : 0, team: o.commentAs.team ? 1 : 0, text: i.value.trim() }); await save({ comments_json: JSON.stringify(cm) }); o.refresh && o.refresh(); };
    const vend = $('[data-vend]', el); if (vend) vend.onclick = (e) => { e.preventDefault(); o.onVendor(p.vendor); };
