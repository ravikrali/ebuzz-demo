...
(function () {
  ...
  const chat = EB.shell({
    ...
    rail: [
      ...
      { id: 'returns', label: 'Returns', icon: 'ret' },
    ...
    onNav: (id) => { EB.view('chat'); ask(LABELS[id], id); },
  });
  EB.setNav('dash');
  ...
  chat.bot([`<h2 style="font-size:24px;margin-bottom:6px">Supplier Hub</h2><p>Hi Priya. I'm your Supplier Copilot. Here's what's happening at <b>Sitwell Home Co.</b></p>`], { delay: 300 });
  W.dash();
  EB.setSuggest(Object.entries(LABELS).map(([id, label]) => ({ label, id })), (c) => { EB.setNav(c.id); ask(c.label, c.id); });
})();
